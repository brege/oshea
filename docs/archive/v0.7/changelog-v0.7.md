...
## 0.7.9 (Conceptual - Implement `archetype` Command)

### Added

* **New CLI Command: `md-to-pdf-cm archetype <source_collection_name>/<source_plugin_id> <new_archetype_name> [--target-dir <path>]`**
    * Allows users to create a customizable copy (archetype) of an existing plugin from a downloaded collection.
    * The source plugin is specified by its `collection_name/plugin_id`.
    * The new archetype is created with the `new_archetype_name`.
    * By default, archetypes are created in a `my-plugins/` directory located at the same level as the `collections/` root (e.g., `~/.local/share/md-to-pdf/my-plugins/`).
    * The `--target-dir <path>` option allows specifying a custom base directory for the new archetype.
* **Archetype Processing Logic:**
    * Copies the source plugin directory to the target archetype location.
    * Renames the main configuration file (e.g., `source_plugin_id.config.yaml` to `new_archetype_name.config.yaml`).
    * Updates the `description` in the new config file to indicate it's an archetype and references the original source plugin ID.
    * Conventionally renames the primary CSS file (if named `source_plugin_id.css` or if it's the single, simply named CSS file listed) to `new_archetype_name.css` and updates its reference in the config.
    * Conventionally renames the handler script (if named `source_plugin_id.js` and not `index.js`) to `new_archetype_name.js` and updates its reference in the config.
    * Conventionally renames an example Markdown file (if named `source_plugin_id-example.md` or `source_plugin_id_example.md`) to `new_archetype_name-example.md`.
    * Appends/prepends an informational note to the archetype's `README.md` (if one exists).
    * Performs string replacements within common text files (`.js`, `.yaml`, `.yml`, `.css`, `.md`) in the new archetype directory, replacing occurrences of `sourcePluginId` and its PascalCase version with `newArchetypeName` and its PascalCase version. The descriptive fields generated by the archetype command itself (e.g. the new config description, the README note) are protected from this replacement to preserve correct attribution to the original source.
* Added `toPascalCase` utility to `cm-utils.js`.
* Added comprehensive unit tests for the `archetype` command, covering successful operations, error handling, file renames, and content modifications.

### Changed

* Default directory name for user-created archetypes (when `--target-dir` is not used with the `archetype` command) is now `my-plugins` (e.g. `~/.local/share/md-to-pdf/my-plugins/`) instead of `custom_plugins_user`. This change is reflected in `constants.js`.
