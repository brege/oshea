...
class DependencyTreeTracer {
  constructor(options = {}) {
    ...
    this.presetContextFiles = [...defaultPresets, ...(options.additionalPresets || [])];
    this.presetContextSet = new Set(this.presetContextFiles.map(p => p.replace(/\/$/, '')));
  }
    ...
    walk.simple(ast, {
      VariableDeclarator: (node) => {
        if (node.init && node.init.type === 'CallExpression' && node.init.callee.name === 'require' &&
                    node.init.arguments.length === 1 && node.init.arguments[0].type === 'Literal' &&
    ...
    walk.simple(ast, {
      CallExpression: (node) => {
        if (node.callee.name === 'require' && node.arguments.length > 0) {
          ...
          if (requirePath) {
            if (!requirePath.startsWith('.') && !requirePath.startsWith('/') && requirePath !== '@paths' && !path.isAbsolute(requirePath)) {
              return;
            }
  ...
  _traceDependencies(startFile, visited) {
    ...
    for (const req of requires) {
      ...
      if (resolvedPath) {
        const dependency = this._traceDependencies(path.relative(this.projectRoot, resolvedPath), visited);
  ...
  _buildContextPackage(startFiles) {
    ...
    trees.forEach(tree => this._collectFilesFromTree(tree, allFiles, registryVars));

    this.presetContextFiles.forEach(preset => {
      if (preset.endsWith('/')) {
        this._findFilesInDir(preset).forEach(file => allFiles.add(file.replace(/\\/g, '/')));
  ...
  _generateImportStatement(contextPackage) {
    ...
    for (let i = 0; i < registryVars.length; i += varsPerLine) {
      ...
      lines.push(`  ${chunk.join(', ')}${i + varsPerLine < registryVars.length ? ',' : ''}`);
    }
