...
class RecipeBookHandler {
  ...
  async generate(
    ...
    globalConfig,
    outputDir,
    outputFilenameOpt = 'recipe-book.pdf',
    pluginBasePath,
  ) {
    ...
    if (!recipesBaseDir) {
      throw new Error(
        "The 'recipesBaseDir' option was not provided for the recipe-book plugin. Use --recipes-base-dir <path> with the 'generate' command.",
      );
    }
    ...
    const outputPdfFilename =
      outputFilenameOpt ||
      pluginSpecificConfig.default_filename ||
      'recipe-book.pdf';
    ...
    if (collectedRecipeDetails.length === 0) {
      logger.warn(
        `WARN: No recipe files (index.md in subdirectories) found in ${recipesBaseDir}. The book might be empty or only contain a cover/ToC.`,
      );
    }
    ...
    if (pluginSpecificConfig.cover_page?.enabled) {
      ...
      if (title)
        combinedMarkdown += `  <h1 class="cover-title">${title}</h1>\n`;
      if (subtitle)
        combinedMarkdown += `  <p class="cover-subtitle">${subtitle}</p>\n`;
      if (author)
        combinedMarkdown += `  <p class="cover-author">${author}</p>\n`;
    ...
    for (const recipeDetail of collectedRecipeDetails) {
      ...
      const { data: frontMatter, content: contentWithoutFm } =
        extractFrontMatter(rawRecipeContent);
    ...
    for (const cssFileName of cssFilesToLoad) {
      ...
      if (fss.existsSync(cssFilePath)) {
        ...
        logger.warn(
          `WARN: CSS file for recipe book plugin not found: ${cssFilePath}`,
        );
      }
    }
    if (cssFileContentsArray.length === 0 && cssFilesToLoad.length > 0) {
      logger.warn(
        `WARN: No CSS files were actually loaded for recipe book plugin, though some were specified: ${cssFilesToLoad.join(', ')}.`,
      );
    }
