...
function showUIElements() {
  ...
  console.log(
    `Processing ${theme.highlight('convert')} command for: ${colorize.path('src/docs/example.md')}`,
  );
  console.log(
    `Plugin ${colorize.value('cv')} loaded from ${colorize.path('plugins/cv/')}`,
  );
  console.log(
    `Found ${colorize.value('3')} enabled plugins in ${colorize.context('CollectionsManager')}`,
  );
...
function showValidatorOutput() {
  ...
  console.log(
    theme.success('      ✓ should convert cv-example.md to cv-example.pdf'),
  );
...
function showTableSample() {
  ...
  console.log(
    `${theme.header('Name'.padEnd(15))} ${theme.header('Status'.padEnd(12))} ${theme.header('Source')}`,
  );
  console.log(theme.border('─'.repeat(50)));
  console.log(
    `${'cv'.padEnd(15)} ${theme.enabled('Enabled'.padEnd(12))} Bundled`,
  );
  console.log(
    `${'recipe'.padEnd(15)} ${theme.enabled('Enabled'.padEnd(12))} Bundled`,
  );
  console.log(
    `${'business-card'.padEnd(15)} ${theme.disabled('Available'.padEnd(12))} CM`,
  );
  console.log(
    `${'invoice'.padEnd(15)} ${theme.pending('Pending'.padEnd(12))} CM`,
  );
  console.log(theme.border('─'.repeat(50)));
  console.log('');
}
...
function showCommandExamples() {
  ...
  console.log(
    `  Plugin ${colorize.value('cv')} ${theme.context('determined via CLI option')}`,
  );
  ...
  console.log(
    `  ${theme.disabled('○')} business-card ${theme.context('(CM: Available)')}`,
  );
  console.log('');
}
...
function main() {
  ...
  console.log(
    theme.debug(
      'Use this to test color combinations and formatting before implementing in the app.',
    ),
  );
}
