...
const { findVariableByPath, getPathByVariable, getAllPathVariables } = require(pathFinderPath);
...
allVars.slice(0, 10).forEach(item => {
  console.log(`  ${item.variable} → ${path.relative(process.cwd(), item.path)}`);
});
...
testCircularPaths.forEach(originalPath => {
  ...
  if (variables.length > 0) {
    ...
    if (backToPath.length > 0 && path.resolve(backToPath[0].path) === path.resolve(originalPath)) {
      console.log(`✓ ${originalPath} ↔ ${variable} (circular lookup successful)`);
    } else {
...
const duplicates = Object.entries(variableCount).filter(([, count]) => count > 1);
...
sampleVars.forEach(variable => {
  ...
  if (results.length > 0) {
    ...
    if (fs.existsSync(resolvedPath)) {
      console.log(`✓ ${variable} → file exists at ${path.relative(process.cwd(), resolvedPath)}`);
    } else {
      console.log(`✗ ${variable} → file missing at ${path.relative(process.cwd(), resolvedPath)}`);
    }
  }
});
