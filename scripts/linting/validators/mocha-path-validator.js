...
function runValidator({
  ...
  json = false,
  debug = false,
  fix = false,    // not used, stub for interface
  force = false,  // not used, stub for interface
  config = {}
} = {}) {
  if (debug || (!quiet && !json)) {
  ...
  if (!fs.existsSync(mocharcPath)) {
    ...
    process.exitCode = 1;
    return { error: msg, missing: true, path: mocharcPath, summary: { found: 0, missing: 1 }, results: [] };
  }
...
if (require.main === module) {
  ...
  const config = loadLintSection('mochaPathValidator', lintingConfigPath) || {};
  runValidator({
    quiet: !!flags.quiet,
    json: !!flags.json,
    debug: !!flags.debug,
