// dev/src/collections-manager/test/disable.test.js
...
async function testDisablePlugin(testRunStats) {
    ...
    try {
        ...
        // For v0.7.5, options.as will be options.name
        ...
        // For v0.7.5, options.as will be options.name
        await manager.enablePlugin(`${mockCollectionName}/${mockPluginId}`, { name: 'anotherPlugin' });
...
async function runDisableTests(testRunStats) {
    await testDisablePlugin(testRunStats);
}
