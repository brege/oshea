...
module.exports = async function addSingletonPlugin(
  dependencies,
  sourcePluginPath,
  options = {},
) {
  ...
  if (!fss.lstatSync(sourcePluginPath).isDirectory()) {
    ...
    throw new Error(
      `Source plugin path is not a directory: "${sourcePluginPath}"`,
    );
  }
  ...
  if (!cmUtils.isValidPluginName(pluginId)) {
    ...
    throw new Error(
      `Source plugin directory name "${pluginId}" is not a valid plugin name (alphanumeric and hyphens, not at start/end).`,
    );
  }

  const potentialConfigs = [`${pluginId}.config.yaml`, `${pluginId}.yaml`];
  const foundConfig = potentialConfigs.find((cfg) =>
    fss.existsSync(path.join(sourcePluginPath, cfg)),
  );

  if (!foundConfig) {
    const altConfigs = fss
      .readdirSync(sourcePluginPath)
      .filter((f) => f.endsWith('.config.yaml') || f.endsWith('.yaml'));
    if (altConfigs.length === 0) {
      ...
      throw new Error(
        `Source directory "${sourcePluginPath}" does not appear to be a valid plugin: missing a recognized '*.config.yaml' or '*.yaml' file.`,
      );
    }
    logger.warn(
      'Plugin config not found by standard name, but other config files exist',
      {
        context: 'AddSingletonPluginCommand',
        sourcePluginPath: sourcePluginPath,
  ...
  const invokeName = options.name || pluginId;
  if (!cmUtils.isValidPluginName(invokeName)) {
    ...
    throw new Error(
      `Invalid invoke_name: "${invokeName}". Must be alphanumeric and hyphens (not at start/end).`,
    );
  }

  const enabledManifest = await this._readEnabledManifest();
  if (
    enabledManifest.enabled_plugins.some((p) => p.invoke_name === invokeName)
  ) {
    ...
    throw new Error(
      `Invoke name "${invokeName}" is already in use. Please choose a different name using --name.`,
    );
  }
  ...
  if (fss.existsSync(targetPluginDir)) {
    logger.error(
      'A plugin with this ID already exists in user-added plugins directory',
      {
        context: 'AddSingletonPluginCommand',
        pluginId: pluginId,
    ...
    throw new Error(
      `A plugin with ID "${pluginId}" already exists in the user-added plugins directory: "${targetPluginDir}". Remove it first or choose a different source plugin directory name.`,
    );
  }
  ...
  try {
    ...
    await fsExtra
      .rm(targetPluginDir, { recursive: true, force: true })
      .catch((err) => {
  ...
  await this._addToUserPluginsManifest(
    singletonsBaseDir,
    pluginId,
    invokeName,
    sourcePluginPath,
  ...
  logger.success(
    `Singleton plugin added and enabled: ${pluginId} as ${invokeName}`,
    {
      context: 'AddSingletonPluginCommand',
      source: sourcePluginPath,
