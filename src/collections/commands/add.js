...
module.exports = async function addCollection(
  dependencies,
  source,
  options = {},
) {
  ...
  if (fss.existsSync(targetPath)) {
    ...
    throw new Error(
      `Error: Target directory '${targetPath}' already exists. Please remove it or choose a different name.`,
    );
  }
  ...
  if (
    /^(http(s)?:\/\/|git@)/.test(source) ||
    (typeof source === 'string' &&
      source.endsWith('.git') &&
      fss.existsSync(path.resolve(source)))
  ) {
    const sourceToClone = /^(http(s)?:\/\/|git@)/.test(source)
      ? source
      : path.resolve(source);
    logger.info(
      `Source is a Git repository, attempting to clone: ${sourceToClone}`,
      {
        context: 'AddCollectionCommand',
      },
    );
    await this._spawnGitProcess(
      ['clone', sourceToClone, targetPath],
      this.collRoot,
      `cloning ${collectionName}`,
    );
    logger.success(
      `Successfully cloned repository: ${sourceToClone} -> ${targetPath}`,
      {
        context: 'AddCollectionCommand',
      },
    );
    await this._writeCollectionMetadata(
    ...
    logger.info(
      `Source is a non-Git local path, attempting to copy: ${absoluteSourcePath}`,
      {
        context: 'AddCollectionCommand',
      },
    );
    ...
    try {
      ...
      logger.success(
        `Successfully copied local source: ${absoluteSourcePath} -> ${targetPath}`,
        {
          context: 'AddCollectionCommand',
        },
      );
