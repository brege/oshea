...
module.exports = async function enablePlugin(
  dependencies,
  collectionPluginId,
  options = {},
) {
  ...
  if (parts.length !== 2) {
    ...
    throw new Error(
      `Invalid format for collectionPluginId: "${collectionPluginId}". Expected "collection_name/plugin_id".`,
    );
  }
  ...
  const pluginToEnable = availablePlugins.find(
    (p) => p.plugin_id === pluginId && p.collection === collectionName,
  );
  ...
  if (!pluginToEnable) {
    ...
    throw new Error(
      `Plugin "${pluginId}" in collection "${collectionName}" is not available or does not exist.`,
    );
  }
  if (
    !pluginToEnable.config_path ||
    !fss.existsSync(pluginToEnable.config_path)
  ) {
    ...
    throw new Error(
      `Config path for plugin "${pluginId}" in collection "${collectionName}" is invalid or not found: ${pluginToEnable.config_path}`,
    );
  }
  ...
  if (!options.bypassValidation) {
    ...
    if (!validationResult.isValid) {
      ...
      throw new Error(
        `Plugin validation failed for '${pluginId}'. Errors:\n  - ${errorMessages}`,
      );
    }
  ...
  if (!/^[a-zA-Z0-9_.-]+$/.test(invokeName)) {
    ...
    throw new Error(
      `Invalid invoke_name: "${invokeName}". Must be alphanumeric, underscores, hyphens, or periods.`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Invoke name "${invokeName}" is already in use. Choose a different name using --name.`,
    );
  }
  ...
  enabledManifest.enabled_plugins.sort((a, b) =>
  ...
  return {
    success: true,
    message: `Plugin "${collectionName}/${pluginId}" enabled as "${invokeName}".`,
    invoke_name: invokeName,
  };
};
