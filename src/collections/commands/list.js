...
module.exports = async function listCollections(
  dependencies,
  type = 'downloaded',
  ...
  if (type === 'downloaded' || type === 'collections') {
    try {
      ...
      const entries = fss.existsSync(collectionsPath)
      ...
      const legacyUserPluginsPath = path.join(
        this.collRoot,
        collectionsUserPluginsDirname,
      );
      if (fss.existsSync(legacyUserPluginsPath)) {
        const singletons = await fs.readdir(legacyUserPluginsPath, {
          withFileTypes: true,
        });
      ...
      for (const dirent of entries) {
        if (dirent.isDirectory()) {
          ...
          if (collectionName === collectionsUserPluginsDirname) {
            const singletonPluginsContainerPath = path.join(
              this.collRoot,
              collectionsUserPluginsDirname,
            );
            if (fss.existsSync(singletonPluginsContainerPath)) {
              const singletons = await fs.readdir(
                singletonPluginsContainerPath,
                { withFileTypes: true },
              );
              if (singletons.some((sDirent) => sDirent.isDirectory())) {
                collectionInfos.push({
            ...
            const metadata = await this._readCollectionMetadata(
              path.join('collections', collectionName),
            );
            collectionInfos.push({
              name: collectionName,
              source:
                metadata?.source || 'N/A (Metadata missing or unreadable)',
    ...
    if (collectionNameFilter) {
      pluginsFromManifest = pluginsFromManifest.filter(
        (p) => p.collection_name === collectionNameFilter,
      );
    ...
    for (const p of pluginsFromManifest) {
      ...
      if (
        pluginEntry.original_source &&
        pluginEntry.collection_name === collectionsUserPluginsDirname
      ) {
    ...
    processedEnabledPlugins.sort((a, b) =>
      (a.invoke_name || '')
        .toLowerCase()
        .localeCompare((b.invoke_name || '').toLowerCase()),
    );
    ...
    logger.warn('Listing type is not implemented', {
      ...
      listType: type,
      suggestion:
        'Supported types are "downloaded", "collections", "available", "all", "enabled".',
    });
    return [];
  }
};
