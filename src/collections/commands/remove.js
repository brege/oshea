...
module.exports = async function removeCollection(
  ...
  const { fss, path, fsExtra, logger, collectionsUserPluginsDirname } =
    dependencies;
  ...
  const collectionsDir = this.collRoot.endsWith('collections')
    ? this.collRoot
  ...
  if (!fss.existsSync(collectionPath)) {
    logger.warn('Collection path does not exist', {
      ...
      collectionName: collectionName,
      suggestion:
        'Will proceed to check manifest for cleanup if --force is used.',
    });
    ...
    throw new Error(
      `Target "${collectionName}" at ${collectionPath} is not a directory.`,
    );
  }
  ...
  const pluginsFromThisCollectionCount = enabledManifest.enabled_plugins.filter(
    (p) => {
      if (specificPluginIdForFilterCheck) {
        return (
          p.collection_name === actualCollectionNameForFilterCheck &&
          p.plugin_id === specificPluginIdForFilterCheck
        );
      }
  ...
  if (pluginsFromThisCollectionCount > 0 && !options.force) {
    const enabledPluginsDetails = enabledManifest.enabled_plugins
      .filter((p) => {
        if (specificPluginIdForFilterCheck) {
          return (
            p.collection_name === actualCollectionNameForFilterCheck &&
      ...
      .map(
        (p) => `"${p.invoke_name}" (from ${p.collection_name}/${p.plugin_id})`,
      )
      .join(', ');
    ...
    throw new Error(
      `Collection "${collectionName}" has enabled plugins: ${enabledPluginsDetails}. Please disable them first or use the --force option.`,
    );
  }

  if (options.force && pluginsFromThisCollectionCount > 0) {
    logger.warn(
      'Force removing collection, disabling associated plugins first',
      {
        context: 'RemoveCollectionCommand',
        collectionName: collectionName,
  ...
  if (
    ...
    logger.info(
      'Collection directory not found for physical removal, assuming already removed or non-existent.',
      {
        context: 'RemoveCollectionCommand',
        collectionName: collectionName,
        collectionPath: collectionPath,
  ...
  return {
    success: true,
    message: `Collection "${collectionName}" processed for removal.`,
  };
};
