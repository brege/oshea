...
class PluginConfigLoader {
  ...
  async applyOverrideLayers(pluginName, layer0ConfigData, contributingPaths) {
    ...
    if (this.useFactoryDefaultsOnly) {
      return {
        mergedConfig: currentMergedConfig,
        mergedCssPaths: currentCssPaths,
        contributingPaths,
      };
    }
    ...
    if (this.fs.existsSync(xdgPluginOverrideFilePath)) {
      const layer1Data = await this._loadSingleConfigLayer(
    ...
    if (
      this.xdgMainConfig[pluginName] &&
      this.configUtils.isObject(this.xdgMainConfig[pluginName])
    ) {
      const inlineXdgOverrideBlock = this.xdgMainConfig[pluginName];
      currentMergedConfig = this.configUtils.deepMerge(
        currentMergedConfig,
        inlineXdgOverrideBlock,
      );
      // Use the stored xdgMainConfigPath for accurate reporting
      const xdgConfigReportPath = this.xdgMainConfigPath
        ? this.xdgMainConfigPath
        : this.path.join(
            this.xdgBaseDir || '~/.config/oshea',
            'config.yaml (path not found)',
          );
      contributingPaths.push(
        `Inline override from XDG main config: ${xdgConfigReportPath}`,
      );
      currentCssPaths = this.AssetResolver.resolveAndMergeCss(
    ...
    if (this.projectMainConfig && this.projectBaseDir) {
      // projectBaseDir is dirname of projectMainConfigPath
      // Project File-based Override (via plugins: key in the --config file)
      if (
        this.projectMainConfig.plugins &&
        typeof this.projectMainConfig.plugins[pluginName] === 'string'
      ) {
        const projectOverrideRelPath =
          this.projectMainConfig.plugins[pluginName];
        let projectOverrideAbsPath = projectOverrideRelPath;
        if (
          projectOverrideRelPath.startsWith('~/') ||
          projectOverrideRelPath.startsWith('~\\')
        ) {
          projectOverrideAbsPath = this.path.join(
            this.os.homedir(),
            projectOverrideRelPath.substring(2),
          );
          ...
          projectOverrideAbsPath = this.path.resolve(
            this.projectBaseDir,
            projectOverrideRelPath,
          );
        }

        if (
          this.fs.existsSync(projectOverrideAbsPath) &&
          projectOverrideAbsPath !== layer0ConfigData.actualPath
        ) {
          const projectOverrideAssetBase = this.path.dirname(
            projectOverrideAbsPath,
          );
          const layer2Data = await this._loadSingleConfigLayer(
            projectOverrideAbsPath,
          ...
          if (
            ...
            contributingPaths.push(
              `${projectOverrideAbsPath} (empty or no effective overrides)`,
            );
          }
        } else if (
          projectOverrideRelPath &&
      ...
      if (
        this.projectMainConfig[pluginName] &&
        this.configUtils.isObject(this.projectMainConfig[pluginName])
      ) {
        const inlineProjectOverrideBlock = this.projectMainConfig[pluginName];
        currentMergedConfig = this.configUtils.deepMerge(
          currentMergedConfig,
          inlineProjectOverrideBlock,
        );
        // Use the stored projectMainConfigPath for accurate reporting
        const projectConfigReportPath = this.projectMainConfigPath
          ? this.projectMainConfigPath
          : this.path.join(
              this.projectBaseDir || '.',
              'config.yaml (path not found)',
            );
        contributingPaths.push(
          `Inline override from project main config: ${projectConfigReportPath}`,
        );
        currentCssPaths = this.AssetResolver.resolveAndMergeCss(
