...
async function loadConfig(configPath) {
  ...
  try {
    ...
    if (!config) {
      // Handles empty or effectively null config after parsing
      ...
      throw new Error(
        `Configuration file '${configPath}' is empty or invalid.`,
      );
    }
...
function getTypeConfig(fullConfig, docType) {
  const typeSettings =
    fullConfig.document_types?.[docType] || fullConfig.document_types?.default;
  if (!typeSettings) {
    ...
    throw new Error(
      `No configuration found for document type '${docType}' or for 'default'. Missing 'document_types.${docType}' or 'document_types.default' in config.yaml.`,
    );
  }
  ...
  if (
    !Array.isArray(resolvedShortcodePatterns) ||
    resolvedShortcodePatterns.length === 0
  ) {
...
function extractFrontMatter(markdownContent) {
  try {
    ...
    return {
      data: result.data || {},
      content: result.content || markdownContent,
    };
  } catch (e) {
    logger.warn('Could not parse front matter', {
      ...
      error: e.message,
      suggestion:
        'Proceeding with full content as body, no front matter applied.',
    });
...
function resolvePath(object, pathStr) {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (i === 0) {
      ...
      actualKeyInCurrentObject = Object.keys(current).find(
        (k) => k.toLowerCase() === keySegment.toLowerCase(),
      );
      if (actualKeyInCurrentObject === undefined) return undefined;
    } else {
      // For subsequent keys, be case-sensitive (for nested object properties)
      if (Object.hasOwn(current, keySegment)) {
...
function substituteAllPlaceholders(mainContent, initialContextData) {
  ...
  while (fmChangedInLoop && loopCount < maxFmSubstLoops) {
    ...
    for (const key in processingContext) {
      if (
        Object.hasOwn(processingContext, key) &&
        typeof processingContext[key] === 'string'
      ) {
  ...
  const { newContent: processedMainContent } = substitutePlaceholdersInString(
    mainContent,
    processingContext,
    'main content',
  );
  ...
  return {
    processedFmData: processingContext,
    processedContent: processedMainContent,
  };
}
