...
function createMathIntegration(dependencies = {}) {
  ...
  const fss = dependencies.fsSync || require('node:fs');

  const katexPluginModule =
    dependencies.katexPluginModule || require('@vscode/markdown-it-katex');
  ...
  function configureMarkdownItForMath(mdInstance, mathConfig) {
    if (mathConfig?.enabled && mathConfig.engine === 'katex') {
      ...
      if (typeof katexPluginModule === 'function') {
        ...
        katexPluginModule &&
        typeof katexPluginModule.default === 'function'
      ) {
        ...
        logger.error('KaTeX plugin is not a function', {
          ...
          error:
            'Resolved KaTeX plugin or its .default is not a function. Cannot apply to markdown-it.',
      ...
      if (fss.existsSync(KATEX_CSS_PATH)) {
        try {
          ...
          logger.warn('Could not read KaTeX CSS file', {
            ...
            error: error.message,
            suggestion:
              'Ensure the KaTeX CSS file is accessible and not corrupt.',
          });
          return [];
        }
      } else {
        logger.warn('KaTeX CSS file not found', {
          ...
          suggestion:
            'Math rendering might not be styled correctly. Ensure the KaTeX CSS path is correct.',
        });
        return [];
      }
    }
