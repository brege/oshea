...
async function setupWatch(
  ...
  const collectWatchablePaths = async (currentConfigResolver, currentArgs) => {
    ...
    try {
      ...
      const effectiveConfig =
        await configResolver.getEffectiveConfig(pluginSpec);
      ...
      if (
        ...
        Array.isArray(pluginSpecificConfig.watch_sources)
      ) {
        for (const sourceEntry of pluginSpecificConfig.watch_sources) {
          ...
          if (
            sourceEntry.path_from_cli_arg &&
            typeof sourceEntry.path_from_cli_arg === 'string'
          ) {
            ...
            if (cliValue && typeof cliValue === 'string') {
              ...
              logger.warn(
                'CLI argument not found or invalid for watch_sources',
                {
                  context: 'WatchHandler',
            ...
            for (;;) {
              ...
              if (currentArgs[argName] !== undefined) {
                ...
                logger.warn(
                  'CLI argument in placeholder not found for watch_sources path',
                  {
                    context: 'WatchHandler',
                    cliArgument: argName,
            ...
            sourceEntry.type === 'glob' &&
            sourceEntry.pattern &&
            typeof sourceEntry.pattern === 'string'
          ) {
            let globPattern = sourceEntry.pattern;
            let globBase = pluginBasePath;

            if (
              sourceEntry.base_path_from_cli_arg &&
              currentArgs[sourceEntry.base_path_from_cli_arg]
            ) {
              globBase = path.resolve(
                currentArgs[sourceEntry.base_path_from_cli_arg],
              );
            }
            ...
            for (;;) {
              ...
              if (currentArgs[argName] !== undefined) {
                ...
                logger.warn(
                  'CLI argument in placeholder not found for watch_sources glob pattern',
                  {
                    context: 'WatchHandler',
                    cliArgument: argName,
          ...
          if (resolvedPathToAdd) {
            if (
              sourceEntry.type !== 'glob' &&
              !fs.existsSync(resolvedPathToAdd)
            ) {
              logger.warn('Declared watch path does not exist', {
                ...
                plugin: currentArgs.plugin || currentArgs.pluginName,
                suggestion:
                  'Verify the path specified in plugin configuration exists.',
              });
            } else {
            ...
            logger.warn(
              'Could not resolve path for a watch_sources entry in plugin',
              {
                context: 'WatchHandler',
                plugin: currentArgs.plugin || currentArgs.pluginName,
                sourceEntry: JSON.stringify(sourceEntry),
                suggestion:
                  'Check the format and values of the watch_sources entry.',
              },
            );
          }
        }
      ...
      if (
        configSources.mainConfigPath &&
        fs.existsSync(configSources.mainConfigPath)
      ) {
      ...
      if (
        effectiveConfig.handlerScriptPath &&
        fs.existsSync(effectiveConfig.handlerScriptPath)
      ) {
      ...
      logger.warn('Could not determine all paths to watch for plugin', {
        ...
        error: error.message,
        suggestion:
          'Falling back to watch Markdown and config files if they exist.',
      });
      if (currentArgs.markdownFile && fs.existsSync(currentArgs.markdownFile)) {
        files.add(path.resolve(currentArgs.markdownFile));
      }
    ...
    if (
      configResolver.defaultMainConfigPath &&
      fs.existsSync(configResolver.defaultMainConfigPath)
    ) {
    ...
    if (
      configResolver.xdgGlobalConfigPath &&
      fs.existsSync(configResolver.xdgGlobalConfigPath)
    ) {
  ...
  const rebuild = async (event, filePathTrigger) => {
    ...
    try {
      ...
      const newWatchedPaths = await collectWatchablePaths(
        newConfigResolverForPaths,
        args,
      );

      const pathsToAdd = newWatchedPaths.filter(
        (p) => !watchedPaths.includes(p),
      );
      const pathsToRemove = watchedPaths.filter(
        (p) => !newWatchedPaths.includes(p),
      );
      ...
      if (event === 'unlink' && watchedPaths.includes(filePathTrigger)) {
        if (fs.existsSync(filePathTrigger)) {
          ...
          logger.debug(
            'Re-added unlinked file back to watch list because it re-appeared',
            {
              context: 'WatchHandler',
  ...
  if (watchedPaths.length === 0) {
    logger.warn(
      'Watch mode activated, but no files could be identified to watch',
      {
        context: 'WatchHandler',
        suggestion:
          'Executing command once as no files are being monitored for changes.',
      },
    );
    try {
      await commandExecutor(args);
  ...
  logger.info(
    'Watch mode active. Initially monitoring these files and directories:',
    {
      context: 'WatchHandler',
