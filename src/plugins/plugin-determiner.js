...
async function determinePluginToUse(
  args,
  { fsPromises, fsSync, path, yaml, markdownUtils, processCwd },
  ...
  if (args.markdownFile) {
    ...
    if (fsSync.existsSync(markdownFilePathAbsolute)) {
      try {
        const rawMarkdownContent = await fsPromises.readFile(
          markdownFilePathAbsolute,
          'utf8',
        );
      ...
      if (fsSync.existsSync(localConfigPath)) {
        try {
          const localConfigContent = await fsPromises.readFile(
            localConfigPath,
            'utf8',
          );
  ...
  const isNameSpec =
    ...
    !(pluginSpec.includes(path.sep) || pluginSpec.startsWith('.'));

  if (
    isNameSpec &&
    markdownFilePathAbsolute &&
    (determinationSource.startsWith('front matter') ||
    ...
    logger.debug(
      'Attempting to self-activate plugin based on name spec and markdown file context',
      {
        context: 'PluginDeterminer',
    ...
    if (
      fsSync.existsSync(potentialPluginConfigPath) &&
      fsSync.statSync(potentialPluginConfigPath).isFile()
    ) {
      ...
      fsSync.existsSync(potentialPluginConfigPathDirect) &&
      fsSync.statSync(potentialPluginConfigPathDirect).isFile()
    ) {
  ...
  if (
    ...
    if (markdownFilePathAbsolute) {
      ...
      logger.debug(
        'Resolved relative plugin path based on current working directory',
        {
          context: 'PluginDeterminer',
          originalPluginSpec: originalPluginSpec,
          resolvedPath: pluginSpec,
