...
function formatJs(level, message, meta = {}) {
  try {
    ...
    const formatted = message
      ...
      .replace(
        /\b(const|let|var|function|return|if|else|for|while|class|import|export|require|module\.exports)\b/g,
      ...
      .replace(
        /(\b\w+)(\s*:)/g,
        (_, key, colon) => jsTheme.property(key) + colon,
      )
      ...
      .replace(/(require\(['"][@\w\-./]+['"]\))/g, (match) =>
        jsTheme.import(match),
      )
      .replace(/(import\s+.*?\s+from\s+['"][@\w\-./]+['"])/g, (match) =>
        jsTheme.import(match),
      );
