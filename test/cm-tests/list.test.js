...
async function testListCollectionsType(testRunStats, baseTestRunDir) {
    ...
    // Use a different name for the collection added from Git to avoid potential conflicts if tests run too quickly
    await manager.addCollection('https://github.com/brege/md-to-pdf-plugins.git', { name: 'plugins1-list-test-cm' });
    await manager.addCollection(localSourcePath, { name: 'plugins2-list-test-cm' });

    try {
        const collections = await manager.listCollections('downloaded');
        assert.ok(collections.includes('plugins1-list-test-cm'), `(${testName}) List should include plugins1-list-test-cm`);
        assert.ok(collections.includes('plugins2-list-test-cm'), `(${testName}) List should include plugins2-list-test-cm`);
        ...
        assert.strictEqual(collections.length, actualCollectionDirs.length, `(${testName}) Should list correct number of downloaded collections. Expected ${actualCollectionDirs.length}, Got ${collections.length}. Dirs: ${actualCollectionDirs.join(', ')} Collections: ${collections.join(', ')}`);
