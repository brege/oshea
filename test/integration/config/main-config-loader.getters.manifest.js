...
module.exports = [
  makeMainConfigLoaderScenario({
    description: '1.4.17: getPrimaryMainConfig() should return the correctly structured primary config object',
    ...
    assertion: async (loader, mocks, constants, expect) => {
      ...
      expect(result.config).to.deep.equal({ isPrimary: true, projectRoot: '/root' });
      expect(result.path).to.equal(path.join(process.cwd(), 'config.yaml'));
      expect(result.reason).to.equal('bundled main');
    },
  }),
  makeMainConfigLoaderScenario({
    description: '1.4.18: getXdgMainConfig() should return the correctly structured XDG config object',
    ...
    assertion: async (loader, mocks, constants, expect) => {
      ...
      expect(result.config).to.deep.equal({ isXdg: true, projectRoot: '/root' });
      expect(result.path).to.equal('/xdg/config/dir/config.yaml');
    },
  }),
  makeMainConfigLoaderScenario({
    description: '1.4.19: getProjectManifestConfig() should return the correctly structured project manifest object',
    ...
    assertion: async (loader, mocks, constants, expect) => {
      ...
      expect(result.config).to.deep.equal({ isProject: true, projectRoot: '/root' });
