...
module.exports = [
  makeDefaultHandlerScenario({
    description: '2.2.1: should successfully process a basic Markdown file and generate a PDF',
    markdownContent: '---\ntitle: Test Doc\n---\n# Hello World',
    stubs: {
      extractFrontMatter: { data: { title: 'Test Doc' }, content: '# Hello World' },
      substituteAllPlaceholders: { processedFmData: { title: 'Test Doc' }, processedContent: '# Hello World' },
  ...
  makeDefaultHandlerScenario({
    description: '2.2.2: should correctly remove shortcodes from Markdown content',
    markdownContent: 'Content with {{< shortcode >}}',
    pluginSpecificConfig: { remove_shortcodes_patterns: ['{{<.*?>}}'] },
    stubs: {
      extractFrontMatter: { data: {}, content: 'Content with {{< shortcode >}}' },
      substituteAllPlaceholders: { processedFmData: {}, processedContent: 'Content with {{< shortcode >}}' },
      removeShortcodes: 'Content with ',
      renderMarkdownToHtml: '<p>Content with </p>',
      generateSlug: { ...alwaysTestSlug },
      expectations: (stubs, expect) => {
        expect(stubs.removeShortcodesStub.calledWith('Content with {{< shortcode >}}', ['{{<.*?>}}'])).to.be.true;
      }
    },
  ...
  makeDefaultHandlerScenario({
    ...
    stubs: {
      ...
      substituteAllPlaceholders: { processedFmData: { name: 'World' }, processedContent: 'Hello World!' },
      removeShortcodes: 'Hello World!',
  ...
  makeDefaultHandlerScenario({
    description: '2.2.4: should apply markdown_it_options from plugin config to the MarkdownIt instance',
    pluginSpecificConfig: { markdown_it_options: { breaks: true, linkify: false } },
    stubs: {
      generateSlug: { ...alwaysTestSlug },
      expectations: (stubs, expect) => {
        const [, , , , , markdownItOptions] = stubs.renderMarkdownToHtmlStub.getCall(0).args;
        expect(markdownItOptions).to.deep.equal({ breaks: true, linkify: false });
      }
    },
    expectedResult: '/output/test-slug.pdf'
  }),
  ...
  makeDefaultHandlerScenario({
    description: '2.2.5: should integrate math rendering by calling math_integration.getMathCssContent when configured',
    pluginSpecificConfig: { math: { enabled: true } },
    stubs: {
      generateSlug: { ...alwaysTestSlug },
      getMathCssContent: ['/*math css*/'],
      expectations: (stubs, expect) => {
        expect(stubs.getMathCssContentStub.calledWith({ enabled: true })).to.be.true;
  ...
  makeDefaultHandlerScenario({
    description: '2.2.6: should correctly apply markdown-it-anchor and markdown-it-table-of-contents options',
    ...
    stubs: {
      generateSlug: { ...alwaysTestSlug },
      expectations: (stubs, expect) => {
        const [, tocOptions, anchorOptions] = stubs.renderMarkdownToHtmlStub.getCall(0).args;
  ...
  makeDefaultHandlerScenario({
    ...
    stubs: {
      ...
      generateSlug: { ...alwaysTestSlug },
      expectations: (stubs, expect) => {
        const [, , , , , , customPlugins] = stubs.renderMarkdownToHtmlStub.getCall(0).args;
  ...
  makeDefaultHandlerScenario({
    ...
    stubs: {
      generateSlug: { ...alwaysTestSlug },
      expectations: (stubs, expect) => {
        stubs.existsSyncStub.withArgs('/plugins/my-plugin/style.css').returns(true);
        stubs.readFileStub.withArgs('/plugins/my-plugin/style.css', 'utf8').resolves('body { color: red; }');
      }
    },
    expectedResult: '/output/test-slug.pdf'
  }),
  ...
  makeDefaultHandlerScenario({
    description: '2.2.9: should load and pass custom HTML template content to pdf_generator',
    markdownContent: '---\ntitle: Template Test\n---\nContent',
  ...
  makeDefaultHandlerScenario({
    description: '2.2.9: should proceed without a template if html_template_path is not provided',
    stubs: {
      generateSlug: { ...alwaysTestSlug },
      expectations: (stubs, expect) => {
        expect(stubs.generatePdfStub.calledOnce).to.be.true;
  ...
  makeDefaultHandlerScenario({
    ...
    pluginSpecificConfig: { head_html: '<meta>', body_html_start: '<header>', body_html_end: '<footer>' },
    stubs: {
      generateSlug: { ...alwaysTestSlug },
      expectations: (stubs, expect) => {
        expect(stubs.generatePdfStub.calledOnce).to.be.true;
        const [, , , , , injectionPoints] = stubs.generatePdfStub.getCall(0).args;
  ...
  makeDefaultHandlerScenario({
    description: '2.2.10: should pass empty strings for unspecified HTML injection snippets',
    stubs: {
      generateSlug: { ...alwaysTestSlug },
      expectations: (stubs, expect) => {
  ...
  makeDefaultHandlerScenario({
    ...
    markdownContent: '---\ntitle: My Doc\nauthor: Me\n---\nContent',
    stubs: {
      extractFrontMatter: { data: { title: 'My Doc', author: 'Me' }, content: 'Content' },
      substituteAllPlaceholders: { processedFmData: { title: 'My Doc', author: 'Me' }, processedContent: 'Content' },
  ...
  makeDefaultHandlerScenario({
    ...
    stubs: {
      ...
      substituteAllPlaceholders: { processedFmData: { lang: 'fr' }, processedContent: 'Bonjour' },
      generateSlug: { ...alwaysTestSlug },
      expectations: (stubs, expect) => {
        const injectionPoints = stubs.generatePdfStub.getCall(0).args[5];
  ...
  makeDefaultHandlerScenario({
    description: '2.2.14: should default to "en" if lang is not specified in front matter',
    stubs: {
      generateSlug: { ...alwaysTestSlug },
      expectations: (stubs, expect) => {
        const injectionPoints = stubs.generatePdfStub.getCall(0).args[5];
  ...
  makeDefaultHandlerScenario({
    ...
    pluginSpecificConfig: { inject_fm_title_as_h1: true, omit_title_heading: true },
    markdownContent: '---\ntitle: My Title\n---',
    stubs: {
      extractFrontMatter: { data: { title: 'My Title' }, content: '' },
      substituteAllPlaceholders: { processedFmData: { title: 'My Title' }, processedContent: '' },
  ...
  makeDefaultHandlerScenario({
    description: '2.2.15: should inject H1 title if omit_title_heading is false or undefined',
    pluginSpecificConfig: { inject_fm_title_as_h1: true, omit_title_heading: false },
    markdownContent: '---\ntitle: My Title\n---',
    stubs: {
      extractFrontMatter: { data: { title: 'My Title' }, content: '' },
      substituteAllPlaceholders: { processedFmData: { title: 'My Title' }, processedContent: '' },
  ...
  makeDefaultHandlerScenario({
    description: '2.2.12: should gracefully handle errors during Markdown file reading',
    stubs: {
      existsSync: false,
    },
  ...
  makeDefaultHandlerScenario({
    description: '2.2.16.1: should return null and log an error if fs.readFile fails',
    markdownContent: 'any',
    stubs: {
      readFileThrows: true,
      generateSlug: { ...alwaysTestSlug },
  ...
  makeDefaultHandlerScenario({
    description: '2.2.16.2: should return null and log an error if renderMarkdownToHtml fails',
    markdownContent: 'any',
    stubs: {
  ...
  makeDefaultHandlerScenario({
    description: '2.2.16.3: should return null and log an error if generatePdf fails',
    markdownContent: 'any',
    stubs: {
