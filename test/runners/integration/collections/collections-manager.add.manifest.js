// test/runners/integration/collections/collections-manager.add.manifest.js
...
module.exports = [
  makeCollectionsManagerScenario({
    description:
      '2.1.3: should successfully clone a git repository and write metadata',
    ...
    assertion: (result, mocks, constants, expect) => {
      ...
      expect(
        mocks.mockDependencies.fs.mkdir.calledWith(FAKE_COLL_ROOT, {
          recursive: true,
        }),
      ).to.be.true;
      expect(
        mocks._spawnGitProcess.calledWith(
          ['clone', FAKE_REPO_URL, expectedPath],
          FAKE_COLL_ROOT,
          `cloning ${DERIVED_COLLECTION_NAME}`,
        ),
      ).to.be.true;
      expect(
        mocks._writeCollectionMetadata.calledWith(
          DERIVED_COLLECTION_NAME,
          sinon.match({ source: FAKE_REPO_URL }),
        ),
      ).to.be.true;
    },
  }),
  makeCollectionsManagerScenario({
    description:
      '2.1.4: should throw an error if the target collection directory already exists',
    methodName: 'addCollection',
    methodArgs: [FAKE_REPO_URL],
    isNegativeTest: true,
  ...
  makeCollectionsManagerScenario({
    ...
    stubs: {
      ...
      internal: {
        _spawnGitProcess: {
          rejects: new Error('Git clone failed: Repository not found'),
        },
      },
    },
    isNegativeTest: true,
  ...
  makeCollectionsManagerScenario({
    description:
      '2.1.5: should throw an error for a non-existent local source path',
    methodName: 'addCollection',
    ...
    expectedErrorMessage:
      'Local source path does not exist: /resolved/./non-existent-dir',
  }),

  makeCollectionsManagerScenario({
    description:
      '2.1.6: should call deriveCollectionName when no name override is provided',
    methodName: 'addCollection',
    methodArgs: [FAKE_REPO_URL],
    stubs: {
      fss: { existsSync: { returns: false } },
      cmUtils: { deriveCollectionName: DERIVED_COLLECTION_NAME },
      internal: {
        _spawnGitProcess: { resolves: true },
        _writeCollectionMetadata: { resolves: true },
      },
    },
    assertion: (_result, mocks, _constants, expect) => {
      ...
      expect(mocks._spawnGitProcess.firstCall.args[0][2]).to.include(
        DERIVED_COLLECTION_NAME,
      );
    },
  }),
  makeCollectionsManagerScenario({
    description:
      '2.1.6: should use the provided name override instead of deriving one',
    methodName: 'addCollection',
    ...
    assertion: (_result, mocks, constants, expect) => {
      ...
      expect(mocks.mockDependencies.cmUtils.deriveCollectionName.called).to.be
        .false;
      expect(mocks._spawnGitProcess.firstCall.args[0][2]).to.equal(
        `${FAKE_COLL_ROOT}/my-override-name`,
      );
    },
  }),
];
