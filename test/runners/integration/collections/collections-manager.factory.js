    ...
    if (stubs.fs) {
      for (const [func, behavior] of Object.entries(stubs.fs)) {
        if (behavior.resolves !== undefined)
          mockDependencies.fs[func].resolves(behavior.resolves);
        if (behavior.rejects !== undefined)
          mockDependencies.fs[func].rejects(new Error(behavior.rejects));
      }
    }
