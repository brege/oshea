// test/runners/integration/collections/collections-manager.list.manifest.js
...
module.exports = [
  makeCollectionsManagerScenario({
    description:
      '2.1.13: should list all downloaded collections from the file system',
    ...
    stubs: {
      ...
      internal: {
        _readCollectionMetadata: {
          resolves: { name: 'stub-name', source: 'stub-source' },
        },
      },
    },
  ...
  makeCollectionsManagerScenario({
    description:
      '2.1.14 & 2.1.15: should list all enabled plugins, sorted by invoke_name',
    methodName: 'listCollections',
    methodArgs: ['enabled'],
    stubs: {
      fss: {
  ...
  makeCollectionsManagerScenario({
    description:
      '2.1.14 & 2.1.15: should correctly filter the enabled plugins by collection name when provided',
    methodName: 'listCollections',
  ...
  makeCollectionsManagerScenario({
    description:
      "2.1.16: should return an empty array for 'downloaded' type if the collection root does not exist",
    methodName: 'listCollections',
    methodArgs: ['downloaded'],
    stubs: {
  ...
  makeCollectionsManagerScenario({
    description:
      "2.1.16: should return an empty array for 'enabled' type if the enabled manifest is missing or empty",
    methodName: 'listCollections',
    methodArgs: ['enabled'],
    stubs: {
  ...
  makeCollectionsManagerScenario({
    description:
      '2.1.26: should return an empty array for a collection with no directories containing valid plugin config files',
    methodName: 'listAvailablePlugins',
    methodArgs: ['no-plugins-collection'],
    stubs: {
      fs: {
        readdir: {
          resolves: [{ name: 'not-a-plugin', isDirectory: () => true }],
        },
      },
      fss: {
  ...
  makeCollectionsManagerScenario({
    ...
    stubs: {
      ...
      fs: {
        readdir: {
          resolves: [{ name: 'plugin-with-desc', isDirectory: () => true }],
        },
  ...
  makeCollectionsManagerScenario({
    ...
    stubs: {
      ...
      fs: {
        readdir: {
          resolves: [{ name: 'plugin-no-desc', isDirectory: () => true }],
        },
