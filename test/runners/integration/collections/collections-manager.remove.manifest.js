...
module.exports = [
  makeCollectionsManagerScenario({
    description: '2.1.7: should throw an error if the collection has enabled plugins and --force is not used',
    ...
    stubs: {
      ...
      internal: {
        _readEnabledManifest: { resolves: { enabled_plugins: [{ collection_name: COLLECTION_TO_REMOVE }] } },
      },
    },
    assertion: (result, mocks) => {
      expect(mocks.disableAllPluginsFromCollection && mocks.disableAllPluginsFromCollection.called).to.not.be.true;
      expect(mocks.mockDependencies.fsExtra.rm.called).to.be.false;
    },
  }),
  makeCollectionsManagerScenario({
    description: '2.1.7: should remove a collection and disable its plugins when --force is used',
    methodName: 'removeCollection',
    ...
    assertion: (result, mocks, constants) => {
      ...
      expect(mocks.disableAllPluginsFromCollection.calledWith(COLLECTION_TO_REMOVE)).to.be.true;
      expect(mocks.mockDependencies.fsExtra.rm.calledWith(`${FAKE_COLL_ROOT}/${COLLECTION_TO_REMOVE}`)).to.be.true;
    },
  }),
  makeCollectionsManagerScenario({
    description: '2.1.8: should handle attempts to remove a non-existent collection gracefully',
    methodName: 'removeCollection',
