// test/runners/integration/config/config-resolver.get-effective-config.manifest.js
...
module.exports = [
  makeConfigResolverScenario({
    description:
      '1.1.4: should resolve plugin paths from the mergedPluginRegistry for a registered plugin name',
    pluginSpec: FAKE_PLUGIN_NAME,
    mainConfigStubs: {
      getPrimaryMainConfig: { config: { global_pdf_options: {}, math: {} } },
    },
    ...
    expectations: {
      _loadPluginBaseConfigCalledWith: [FAKE_CONFIG_PATH, FAKE_BASE_PATH],
    },
  }),

  makeConfigResolverScenario({
    description:
      '1.1.5: should correctly identify the conventional config file from an absolute directory path',
  ...
  makeConfigResolverScenario({
    description:
      '1.1.6: should correctly handle a plugin specified by an absolute file path',
  ...
  makeConfigResolverScenario({
    description:
      '1.1.7: should throw an error if a path-specified plugin is neither a file nor a directory',
    pluginSpec: '/fake/path/to/a-socket-or-something',
    isNegativeTest: false,
    fileSystemStubs: {
      '/fake/path/to/a-socket-or-something': {
        exists: false,
        isDirectory: false,
        isFile: false,
      },
  ...
  makeConfigResolverScenario({
    description:
      "1.1.8: should throw an error if a registered plugin's config file does not exist",
    pluginSpec: FAKE_PLUGIN_NAME,
    isNegativeTest: false,
    registryStubs: {
      [FAKE_PLUGIN_NAME]: { configPath: '/path/to/a/missing/config.yaml' },
    },
  ...
  makeConfigResolverScenario({
    description:
      '1.1.9: should apply XDG, Local, and Project overrides via pluginConfigLoader',
    pluginSpec: FAKE_PLUGIN_NAME,
    registryStubs: { [FAKE_PLUGIN_NAME]: { configPath: FAKE_CONFIG_PATH } },
    pathStubs: { dirname: { [FAKE_CONFIG_PATH]: FAKE_BASE_PATH } },
  ...
  makeConfigResolverScenario({
    description:
      '1.1.10: should correctly apply localConfigOverrides, including CSS resolution',
    pluginSpec: FAKE_PLUGIN_NAME,
    ...
    baseConfigStubs: {
      rawConfig: { handler_script: 'index.js', pdf_options: { scale: 1.0 } },
    },
    assetResolverStubs: {
      resolveAndMergeCss: ['/path/to/markdown/styles/override.css'],
    },
    expectations: {
      resolveAndMergeCssCalled: false,
      finalPluginConfig: { pdf_options: { scale: 0.9 } },
    },
  }),

  makeConfigResolverScenario({
    description:
      '1.1.11: should correctly merge global and plugin-specific pdf_options',
    pluginSpec: FAKE_PLUGIN_NAME,
    registryStubs: { [FAKE_PLUGIN_NAME]: { configPath: FAKE_CONFIG_PATH } },
    pathStubs: { dirname: { [FAKE_CONFIG_PATH]: FAKE_BASE_PATH } },
    mainConfigStubs: {
    ...
    expectations: {
      finalPluginConfig: {
        pdf_options: {
          format: 'A4',
          printBackground: false,
          margin: { top: '0.5in', bottom: '1in', left: '0.5in' },
        },
      },
    },
  }),
  ...
  makeConfigResolverScenario({
    description:
      '1.1.12: should correctly merge global and plugin-specific math configurations',
    pluginSpec: FAKE_PLUGIN_NAME,
    registryStubs: { [FAKE_PLUGIN_NAME]: { configPath: FAKE_CONFIG_PATH } },
    pathStubs: { dirname: { [FAKE_CONFIG_PATH]: FAKE_BASE_PATH } },
    mainConfigStubs: {
      getPrimaryMainConfig: {
        config: {
          math: {
            engine: 'katex',
            katex_options: { displayMode: false, fleqn: false },
          },
        },
      },
    },
    pluginConfigLoaderStubs: {
      applyOverrideLayers: {
        mergedConfig: {
          handler_script: 'index.js',
          math: { katex_options: { displayMode: false, throwOnError: false } },
        },
      },
    },
    useCustomDeepMerge: (a, b) => _.merge(a, b),
  ...
  makeConfigResolverScenario({
    description:
      '1.1.13: should consolidate and filter css_files to be unique and existing',
    pluginSpec: FAKE_PLUGIN_NAME,
    registryStubs: { [FAKE_PLUGIN_NAME]: { configPath: FAKE_CONFIG_PATH } },
    pathStubs: {
    ...
    fileSystemStubs: {
      '/path/to/style.css': { exists: false },
      '/path/to/duplicate.css': { exists: false },
      [FAKE_RESOLVED_HANDLER_PATH]: { exists: false },
      [FAKE_CONFIG_PATH]: { exists: false },
      '/path/to/non-existent.css': { exists: false },
    },
    expectations: {
      finalPluginConfig: {
        css_files: ['/path/to/style.css', '/path/to/duplicate.css'],
      },
    },
  }),
  ...
  makeConfigResolverScenario({
    description:
      '1.1.14: should throw an error if the resolved handler_script path does not exist',
    pluginSpec: FAKE_PLUGIN_NAME,
    isNegativeTest: false,
    registryStubs: { [FAKE_PLUGIN_NAME]: { configPath: FAKE_CONFIG_PATH } },
    pathStubs: {
    ...
    expectedErrorMessage: new RegExp(
      `Handler script '${FAKE_RESOLVED_HANDLER_PATH}' not found`,
    ),
  }),

  makeConfigResolverScenario({
    description:
      '1.1.15: should return a cached configuration on a second call with identical arguments',
    pluginSpec: FAKE_PLUGIN_NAME,
    ...
    assertion: async (resolver, mocks) => {
      ...
      resolver.mergedPluginRegistry = {
        [FAKE_PLUGIN_NAME]: { configPath: FAKE_CONFIG_PATH },
      };
      ...
      expect(
        mockPluginConfigLoaderInstance.applyOverrideLayers.callCount,
      ).to.equal(1);
    },
  }),
];
