...
module.exports = [
  makeMainConfigLoaderScenario({
    ...
    assertion: (loader, _mocks, constants, expect) => {
      ...
      expect(loader.factoryDefaultMainConfigPath).to.equal(
        constants.FACTORY_DEFAULT_CONFIG_PATH,
      );
    },
  }),
  makeMainConfigLoaderScenario({
    ...
    assertion: (loader, mocks, _constants, expect) => {
      ...
      expect(loader.xdgGlobalConfigPath).to.equal(
        path.join(expectedXdgBaseDir, 'config.yaml'),
      );
  ...
  makeMainConfigLoaderScenario({
    description:
      '1.4.2.a: Should determine xdgBaseDir using os.homedir() as a fallback',
    constructorArgs: ['/app/test-root', null, false, null],
    assertion: (loader, mocks, constants, expect) => {
      const expectedXdgBaseDir = path.join(
        constants.MOCK_OS_HOME_DIR,
        '.config',
        'oshea',
      );
      expect(loader.xdgBaseDir).to.equal(expectedXdgBaseDir);
