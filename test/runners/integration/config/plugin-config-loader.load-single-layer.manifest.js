// test/runners/integration/config/plugin-config-loader.load-single-layer.manifest.js
...
module.exports = [
  makePluginConfigLoaderScenario({
    ...
    assertion: async (result, loader, mocks, constants, expect) => {
      ...
      expect(loader._rawPluginYamlCache).to.have.property(
        '/path/to/plugin/my-plugin.config.yaml-/path/to/plugin',
      );
    },
  }),
  makePluginConfigLoaderScenario({
    ...
    assertion: async (result, loader, mocks, constants, expect) => {
      ...
      expect(mocks.mockDependencies.configUtils.loadYamlConfig.calledOnce).to.be
        .true;
    },
  }),
  makePluginConfigLoaderScenario({
    ...
    methodName: '_loadSingleConfigLayer',
    methodArgs: [
      '/path/to/non-existent.config.yaml',
      '/path/to',
      'non-existent-plugin',
    ],
    ...
    assertion: async (result, loader, mocks, constants, expect, logs) => {
      ...
      expect(
        logs.some(
          (log) =>
            log.level === 'warn' &&
            log.msg.includes('Config file path not provided or does not exist'),
        ),
      ).to.be.true;
    },
  }),
  makePluginConfigLoaderScenario({
    description:
      '1.6.4: should return null if configFilePath is null or undefined',
    constructorArgs: [null, {}, null, null, {}, null, false],
    methodName: '_loadSingleConfigLayer',
    ...
    assertion: async (result, loader, mocks, constants, expect, logs) => {
      ...
      const resultUndefined = await loader._loadSingleConfigLayer(
        undefined,
        '/path/to',
  ...
  makePluginConfigLoaderScenario({
    description:
      '1.6.5: should handle errors during YAML parsing or file reading gracefully, returning an empty config object',
    constructorArgs: [null, {}, null, null, {}, null, false],
    methodName: '_loadSingleConfigLayer',
    ...
    assertion: async (result, loader, mocks, constants, expect, logs) => {
      ...
      expect(
        logs.some(
          (log) =>
            log.level === 'error' &&
            log.msg.includes('Failed to load plugin configuration layer'),
        ),
      ).to.be.true;
    },
  }),
