...
function makeDefaultHandlerScenario({
  ...
  const setup = (DefaultHandler, mockStubs) => {
    ...
    if (stubs.readFileThrows) {
      mockStubs.readFileStub
        .withArgs(markdownFilePath, 'utf8')
        .rejects(new Error(expectedError));
    } else {
      mockStubs.readFileStub
        .withArgs(markdownFilePath, 'utf8')
        .resolves(markdownContent);
    }
    ...
    if (stubs.fileMocks && Array.isArray(stubs.fileMocks)) {
      stubs.fileMocks.forEach((fileMock) => {
        ...
        mockStubs.readFileStub
          .withArgs(mockPath, 'utf8')
          .resolves(fileMock.content);
      });
    }

    // markdown_utils stubs
    mockStubs.extractFrontMatterStub.returns(
      stubs.extractFrontMatter || { data: {}, content: '' },
    );
    mockStubs.substituteAllPlaceholdersStub.returns(
      stubs.substituteAllPlaceholders || {
        processedFmData: {},
        processedContent: '',
      },
    );
    ...
    if (stubs.renderMarkdownToHtmlThrows) {
      ...
      mockStubs.renderMarkdownToHtmlStub.returns(
        stubs.renderMarkdownToHtml || '',
      );
    }
  ...
  const assert = (result, mockStubs, expect, logs) => {
    ...
    if (expectedLogs.length > 0) {
      expect(logs.length).to.equal(
        expectedLogs.length,
        'Number of captured logs must match expected number',
      );
      logs.forEach((log, i) => {
        if (expectedLogs[i]) {
          expect(log.level).to.equal(
            expectedLogs[i].level,
            `Log ${i}: Level mismatch`,
          );
          expect(log.msg).to.equal(
            expectedLogs[i].msg,
            `Log ${i}: Message mismatch`,
          );
          if (expectedLogs[i].data) {
            expect(log.data).to.deep.include(
              expectedLogs[i].data,
              `Log ${i}: Data mismatch`,
            );
          } else {
