...
const { makeBuildRegistryScenario, makeCacheInvalidationScenario } = require(
...
module.exports = [
  {
    description:
      '1.2.22: Should only load from auto-discovered plugins when useFactoryDefaultsOnly is true',
    ...
    ...makeBuildRegistryScenario({
      ...
      assertion: (result, { builderInstance }, _constants, expect) => {
        ...
        expect(builderInstance._getPluginRegistrationsFromFile.notCalled).to.be
          .true;
        expect(builderInstance._getPluginRegistrationsFromCmManifest.notCalled)
          .to.be.true;
  ...
  {
    description:
      '1.2.23: Should merge registrations from bundled, CM, and file sources in the correct order',
    methodName: 'buildRegistry',
    constructorArgs: [
      '/fake/project',
      null,
    ...
    setup: async ({ builderInstance }) => {
      sinon.stub(builderInstance, '_registerBundledPlugins').resolves({
        'bundled-plugin': { sourceType: 'Bundled' },
        'override-me': { sourceType: 'Bundled' },
      });
      sinon
        .stub(builderInstance, '_getPluginRegistrationsFromCmManifest')
        .resolves({
          'cm-plugin': { sourceType: 'CM' },
          'override-me': { sourceType: 'CM' },
        });
      const getFromFileStub = sinon.stub(
        builderInstance,
        '_getPluginRegistrationsFromFile',
      );
      getFromFileStub
        .withArgs(builderInstance.xdgGlobalConfigPath)
        .resolves({ 'xdg-plugin': {} });
      getFromFileStub
  ...
  {
    description:
      "1.2.24: Should return a cached registry if relevant build parameters haven't changed",
    methodName: 'buildRegistry',
    constructorArgs: [
      '/fake/project',
      null,
  ...
  },
  {
    ...makeCacheInvalidationScenario({
      description:
        '1.2.24: Should rebuild the registry if useFactoryDefaultsOnly changes',
  ...
  },
  {
    ...makeCacheInvalidationScenario({
      description:
        '1.2.24: Should rebuild the registry if isLazyLoadMode changes',
      changedConstructorArgs: [
        '/fake/project',
        null,
        null,
  ...
  },
  {
    ...makeCacheInvalidationScenario({
      description:
        '1.2.24: Should rebuild the registry if primaryMainConfigLoadReason changes',
      changedConstructorArgs: [
        '/fake/project',
        null,
        null,
  ...
  },
  {
    ...makeCacheInvalidationScenario({
      description:
        '1.2.24: Should rebuild the registry if projectManifestConfigPath changes',
      changedConstructorArgs: [
        '/fake/project',
        null,
        '/new/path',
  ...
  },
  {
    ...makeCacheInvalidationScenario({
      description:
        '1.2.24: Should rebuild the registry if collectionsManagerInstance changes',
      changedConstructorArgs: [
        '/fake/project',
        null,
        null,
  ...
  {
    description:
      '1.2.25: Should include CM plugins when no CM instance is provided',
    methodName: 'buildRegistry',
  ...
  {
    description:
      '1.2.26: Should handle a missing factoryDefaultMainConfigPath gracefully',
    methodName: 'buildRegistry',
    constructorArgs: ['/fake/project', null, null, false, false, null, null],
    setup: async ({ builderInstance, mockDependencies }) => {
      ...
      mockDependencies.fs.existsSync
        .withArgs(builderInstance.projectManifestConfigPath)
        .returns(false);
      builderInstance.getFromFileSpy = sinon.spy(
        builderInstance,
        '_getPluginRegistrationsFromFile',
      );
      sinon.stub(builderInstance, '_registerBundledPlugins').resolves({});
