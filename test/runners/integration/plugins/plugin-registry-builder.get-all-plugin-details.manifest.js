// test/runners/integration/plugins/plugin-registry-builder.get-all-plugin-details.manifest.js
require('module-alias/register');
const { pluginRegistryBuilderFactoryPath } = require('@paths');
const { makeGetAllPluginDetailsScenario } = require(
...
module.exports = [
  {
    description:
      '1.2.27: Should combine plugins from traditional and CM sources',
    ...
    ...makeGetAllPluginDetailsScenario({
      ...
      cmAvailablePlugins: [
        {
          collection: 'cm-coll',
          plugin_id: 'cm-plugin-1',
          description: 'Available CM Plugin',
        },
        {
          collection: 'cm-coll',
          plugin_id: 'cm-plugin-2',
          description: 'Plugin that will be enabled',
        },
      ],
      cmEnabledPlugins: [
        {
          collection_name: 'cm-coll',
          plugin_id: 'cm-plugin-2',
  ...
  {
    description:
      "1.2.28: Should correctly retrieve and include each plugin's description",
    methodName: 'getAllPluginDetails',
    ...makeGetAllPluginDetailsScenario({
      buildRegistryResult: {
        'traditional-plugin': {
          sourceType: 'Project',
          configPath: '/path/to/traditional.config.yaml',
        },
      },
      cmAvailablePlugins: [
        {
          collection: 'cm-coll',
          plugin_id: 'cm-plugin',
          description: 'A Collections Manager plugin.',
        },
      ],
      cmEnabledPlugins: [
        {
  ...
  {
    description:
      '1.2.29: Should correctly set status and source display for traditional plugins',
    methodName: 'getAllPluginDetails',
    ...makeGetAllPluginDetailsScenario({
      buildRegistryResult: {
        'project-plugin': {
          sourceType: 'Project Manifest (--config)',
          definedIn: '/path/to/project.yaml',
        },
        'xdg-plugin': {
          sourceType: 'XDG Global',
          definedIn: '/path/to/xdg.yaml',
        },
        'bundled-plugin': {
          sourceType: 'Bundled Definitions',
      ...
      assertion: (result, mocks, constants, expect) => {
        ...
        expect(projectPlugin.registrationSourceDisplay).to.equal(
          'Project (--config: project.yaml)',
        );
        expect(xdgPlugin.registrationSourceDisplay).to.equal('XDG (xdg.yaml)');
        expect(bundledPlugin.registrationSourceDisplay).to.equal(
          'Bundled (bundled.yaml)',
        );
  ...
  {
    description:
      "1.2.30: Should correctly distinguish between 'Enabled (CM)' and 'Available (CM)' plugins",
    methodName: 'getAllPluginDetails',
    ...makeGetAllPluginDetailsScenario({
      cmAvailablePlugins: [
        {
  ...
  {
    description:
      '1.2.31: Should handle multiple enabled instances of the same CM plugin',
    methodName: 'getAllPluginDetails',
    ...makeGetAllPluginDetailsScenario({
      cmAvailablePlugins: [
        {
          ...
          description: 'Multi-enabled plugin',
        },
      ],
      cmEnabledPlugins: [
        {
  ...
  {
    description:
      '1.2.32: Should return a list of plugins sorted alphabetically by name',
    methodName: 'getAllPluginDetails',
    ...makeGetAllPluginDetailsScenario({
      buildRegistryResult: {
