...
const { makeFileRegistrationScenario } = require(
...
module.exports = [
  {
    description:
      '1.2.9: Should return an empty object if the main config file does not exist',
    ...
    ...makeFileRegistrationScenario({
      ...
      yamlContent: null,
      expectResult: {},
      expectLogs: [/Main config file for plugin registrations not found/],
    }),
  },
  {
    description:
      '1.2.10: Should return an empty object and log an error if the config file is malformed',
    methodName: '_getPluginRegistrationsFromFile',
    methodArgs: [FAKE_CONFIG_PATH, FAKE_BASE_DIR, 'Test'],
    ...makeFileRegistrationScenario({
      mainConfigPath: FAKE_CONFIG_PATH,
  ...
  {
    description:
      '1.2.11: Should correctly parse a simple config file with plugins',
    methodName: '_getPluginRegistrationsFromFile',
    methodArgs: [FAKE_CONFIG_PATH, FAKE_BASE_DIR, 'Test'],
    ...makeFileRegistrationScenario({
      mainConfigPath: FAKE_CONFIG_PATH,
      ...
      fileSystem: {
        '/fake/plugin.config.yaml': { exists: true, isFile: true },
      },
      expectResult: {
        'test-plugin': {
          configPath: '/fake/plugin.config.yaml',
  ...
  },
  {
    ...
    ...makeFileRegistrationScenario({
      ...
      fileSystem: {
        '/aliased/dir/plugin.config.yaml': { exists: true, isFile: true },
      },
      expectResult: {
        'aliased-plugin': {
  ...
  {
    description:
      '1.2.13: Should skip registration for plugins with unresolved aliases and log a warning',
    methodName: '_getPluginRegistrationsFromFile',
    ...
    ...makeFileRegistrationScenario({
      ...
      resolvedPaths: {
        'bad-alias:plugin.config.yaml': '/fake/bad-alias:plugin.config.yaml',
      },
  ...
  {
    description:
      '1.2.14: Should skip registration for plugin paths that do not exist or are not files and log a warning',
    methodName: '_getPluginRegistrationsFromFile',
    methodArgs: [FAKE_CONFIG_PATH, FAKE_BASE_DIR, 'Test'],
    ...makeFileRegistrationScenario({
      mainConfigPath: FAKE_CONFIG_PATH,
      yamlContent: {
        plugins: { 'nonexistent-plugin': './nonexistent.config.yaml' },
      },
      resolvedPaths: {
        './nonexistent.config.yaml': '/fake/nonexistent.config.yaml',
      },
  ...
  {
    description:
      '1.2.15: Should return an empty object if the config file is empty or contains no plugins/aliases',
    methodName: '_getPluginRegistrationsFromFile',
    methodArgs: [FAKE_CONFIG_PATH, FAKE_BASE_DIR, 'Test'],
    ...makeFileRegistrationScenario({
      mainConfigPath: FAKE_CONFIG_PATH,
  ...
  {
    description:
      '1.2.15: Should skip registration for plugin directories without a suitable config file and log a warning',
    methodName: '_getPluginRegistrationsFromFile',
    methodArgs: [FAKE_CONFIG_PATH, FAKE_BASE_DIR, 'Test'],
    ...makeFileRegistrationScenario({
      mainConfigPath: FAKE_CONFIG_PATH,
  ...
  {
    description:
      '1.2.16: Should log INFO when using an alternative config file within a plugin directory',
    methodName: '_getPluginRegistrationsFromFile',
    methodArgs: [FAKE_CONFIG_PATH, FAKE_BASE_DIR, 'Test'],
    ...makeFileRegistrationScenario({
      mainConfigPath: FAKE_CONFIG_PATH,
      ...
      expectResult: {
        'alt-plugin': {
          configPath: '/fake/alt-dir/alt.config.yaml',
          definedIn: FAKE_CONFIG_PATH,
          sourceType: 'Test',
        },
      },
  ...
  {
    description:
      '1.2.17: Should handle plugin aliases defined in the main config file',
    methodName: '_getPluginRegistrationsFromFile',
    ...
    ...makeFileRegistrationScenario({
      ...
      fileSystem: {
        '/custom/path/plugin.config.yaml': { exists: true, isFile: true },
      },
      expectResult: {
        'test-plugin': {
  ...
  {
    description:
      '1.2.18: Should return empty registrations if no plugins section exists',
    methodName: '_getPluginRegistrationsFromFile',
  ...
  {
    description:
      '1.2.19: Should return empty registrations if plugins section is empty',
    methodName: '_getPluginRegistrationsFromFile',
